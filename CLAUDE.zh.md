...
- 使用 `./04-scripts/manage-skills.sh` 执行注册表和用户级链接操作。保持根目录 `descriptions-zh.conf` 与技能行为一致；注册表或 README 元数据变更后，先运行 `update-readme`，再运行 `check`。
...
3. **手动编辑生成的 README 技能表。** `BEGIN SKILL LIST` 与 `END SKILL LIST` 之间的部分派生自已注册链接和根目录 `descriptions-zh.conf`。使用 `update-readme` 刷新，再用 `check` 验证链接健康状态。
