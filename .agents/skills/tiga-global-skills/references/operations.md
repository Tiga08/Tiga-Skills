...
- `~/.claude/skills` — should be a symlink to `<REPO_ROOT>/03-skills`.
...
- All correct or absent → run `./02-scripts/manage-skills.sh setup` directly.
...
Pre-check that the source path exists and contains `SKILL.md`. If `03-skills/<name>` already exists, the script exits with an error — in that case ask via `AskUserQuestion` with three options: `remove` the old entry first and re-add, register under a different name with `--name`, or cancel.
...
Before confirming, show the entry as `name → target (category)`. Note that the script only removes symlinks (external skills); for real directories (custom skills) it refuses and asks the user to handle manually. The script also removes the matching `descriptions-zh.conf` metadata, but never touches the link target.

## list / check / update-readme

Execute directly. Before `update-readme`, compare the affected skill's current core behavior with its config entry and refresh stale descriptions. If `check` exits non-zero, summarize the failures and suggest fixes by category: for broken links, `remove` the entry or repair the upstream path and re-`add`; for frontmatter violations, edit the source `SKILL.md` (under `03-skills/` or `.agents/skills/`).

## Notes

- `add` converts paths under `$HOME` to user-portable relative symlinks (e.g., `../../../AG-Tools/superpowers/skills/<name>`). This assumes the layout `~/Projects/AG-Tools/tiga-skills` (this repo) and `~/Projects/AG-Tools`; paths outside `$HOME` stay absolute with a portability warning.
...
- `check` verifies every entry under `03-skills/` — symlinks must resolve and have `SKILL.md`; real directories must have `SKILL.md` — plus the project-level links `.claude/skills` / `.codex/skills` → `.agents/skills` and the root `SKILLS-REFS.md` symlink (must resolve to the AG-Tools downstream-reference file). For in-repo sources (custom skills and `.agents/skills/` project-level skills) it also validates frontmatter compliance (name format and consistency with the registered name, description non-empty and ≤1024 characters, no unknown frontmatter keys); external sources skip frontmatter validation. Any failure makes it exit non-zero.
