...
- Entries in `02-agent-skills/` must remain symlinks. Do not place regular skill directories there or directly edit symlink targets.
...
1. **Treating `02-agent-skills/` as a source directory.** It is a registry view; all current entries should be symlinks. To modify registry skills, edit `03-custom-skills/`. For external skills, go to the upstream repository. Project-level skills live in `.agents/skills/`, not here.
...
3. **Forgetting to update the README skill list.** After skill changes, run `./04-scripts/manage-skills.sh update-readme`. This command scans symlinks in `02-agent-skills/` and extracts metadata from each `SKILL.md`.
