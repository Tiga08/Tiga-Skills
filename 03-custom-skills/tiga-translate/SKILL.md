...
**Arguments:** One or more paths separated by spaces. Each path may be a directory or a single `.md` file, mixed freely. 本次调用参数：`$ARGUMENTS`
...
1. Treat every argument as a path — there are no flags to extract.
2. Resolve each path:
...
4. **Assess translation need for each remaining file** — must happen before any translation starts; its result is the plan Phase 3 executes:
   ...
   - Target output file exists → determine the translation's **baseline time**: if it is git-tracked with no uncommitted modifications, use `git log -1 --format=%ai -- <translation>`; if untracked (e.g. `.tiga/translations/` is git-ignored) or has uncommitted modifications, use the file's mtime (`stat`). Then check whether the source changed after that baseline, combining `git status` and `git log`. Unchanged → **already up to date**. Changed → needs an **incremental update**; carry the baseline time forward to Phase 3.
...
- **Incremental update** → run the incremental update flow below, reusing the baseline time computed in Phase 2 step 3.

**Incremental update flow:**

1. **Line-count consistency check.** Compare the total line counts of the source document and the translation. The mapping "source line N ↔ translation line N" is reliable only when the counts match.
2. **Locate changed lines.** Take the last commit that touched the source file before the baseline time as the base, then run `git diff <base> -- <source>` to obtain the changed hunks (this automatically includes uncommitted modifications).
3. **Translate incrementally.** Translate only the lines inside the changed hunks, consulting hunk context to keep terminology and style consistent, and use Edit to replace the corresponding lines in the translation at the same line numbers. For added/removed lines, insert into or delete from the translation following the hunk's line offsets. Stay consistent with renderings already used elsewhere in the existing translation.
