...
Tiga-Skills 通过扁平符号链接注册外部和自定义 Agent Skills，并将它们暴露给 Claude Code 和 Codex。仓库存放内容、Bash 和仅使用标准库的 Python 工具，不含应用代码、构建系统或需通过包管理器安装的运行时依赖。核心约束是：`02-agent-skills/` 是符号链接注册表，自定义技能源文件位于 `03-custom-skills/`，所有注册与移除一律经由 `./04-scripts/manage-skills.sh`。
...
- 保持 `04-scripts/*.sh` 为 UTF-8 编码的可执行 Bash，并沿用既有的直接命令风格；修改任何 shell 脚本后运行 `bash -n`。Skill 自带的 Python 辅助工具通过 `python3` 调用，并遵循其所属 `SKILL.md` 的规则；仓库没有统一测试套件。
...
- 编造技能名称、来源或描述。
- 重新引入已淘汰的顶层布局，例如 `00-skill-index/`、`03-workflows/` 或 `05-custom-skills/`。
