...
Tiga-Skills 是一个内容和脚本仓库，通过符号链接集中注册外部和自定义 Agent Skills，并将它们暴露给 Claude Code 和 Codex。该仓库不包含应用代码、构建系统或运行时依赖。核心约束是：`02-agent-skills/` 只作为符号链接注册表，自定义技能源文件位于 `03-custom-skills/`，并且所有技能注册和移除都必须通过 `./04-scripts/manage-skills.sh` 完成。
...
| `02-agent-skills/` | Agent Skills 注册表；所有条目都应为符号链接 | derived |
...
- 项目级技能用于操作此仓库本身（例如 `manage-skills`）。
...
- 修改文件前先读取相关文件。
- 将 `02-agent-skills/` 视为符号链接注册表；不要直接在其中编辑技能内容。
...
- 手动创建或删除 `02-agent-skills/` 中的符号链接来绕过 `manage-skills.sh`。
- 编造技能名称、来源或描述。
